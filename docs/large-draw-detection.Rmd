...
#system.time(obadiah::draws(s, draw.type="T3", transaction.costs=transaction.costs, discount.rate=discount.rate, tz="Europe/Moscow"))

d01_3 <- obadiah::draws(s, draw.type="T3", transaction.costs=transaction.costs, discount.rate=discount.rate, tz="Europe/Moscow")
t3.draws <- ref.draws[, d01_3[draw.start >= ref.draw.start - minutes(a) & draw.end <= ref.draw.end + minutes(a), ], by=.(ref.draw.end, ref.start.price)]
...
transaction.costs <- 0.0075
discount.rate <- 0.000045

d02_3 <- obadiah::draws(s, draw.type="T3", transaction.costs=transaction.costs, discount.rate=discount.rate, tz="Europe/Moscow")
```


```{r,fig.height=10, fig.width=10}

t02_3.draws <- ref.draws_02[, d02_3[draw.start >= ref.draw.start - minutes(a) & draw.end <= ref.draw.end + minutes(a), ], by=.(ref.draw.end, ref.start.price)]
...
ggplot(ref.spreads_02,
  ...
  #geom_segment(aes(x=ref.draw.start, xend=ref.draw.end, y=ref.start.price, yend=ref.end.price),ref.draws, color="red",
  arrow=arrow(angle=20, length=unit(0.05, "npc"))) + 
  geom_segment(aes(x=draw.start, xend=draw.end, y=log(start.price)-log(ref.start.price), yend=log(end.price)-log(ref.start.price)),t02_3.draws, color="blue") +
  facet_wrap(vars(as.character(ref.draw.end)), scales="free", ncol=3)+ labs(y="Log relative price (draw start == 0.0)", x="Timestamp, GMT+3", title="Reference Set II: Type 3 draws vs reference", subtitle=paste0("Parameters: transaction.costs=", transaction.costs, " discount.rate=", discount.rate))
```
