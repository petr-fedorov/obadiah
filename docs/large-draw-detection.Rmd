...
ref.spreads <- ref.draws[,
          s[timestamp >= ref.draw.start - minutes(1) & timestamp <=  ref.draw.end + minutes(1),
...
d <- obadiah::draws(s, price.source = 'mid-price', draw.type="T2",  min.draw=min.draw, duration=duration,
                             tolerance=tolerance, max.draw=max.draw, tz="Europe/Moscow")
t2.draws <- ref.draws[, d[draw.start >= ref.draw.start - minutes(1) & draw.end <= ref.draw.end + minutes(1), ], by=.(ref.draw.end, ref.start.price)]
